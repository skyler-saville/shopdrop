...
var Shopper = require('../../models/users/shopper').Shopper
...
function validResourceID (req, res) {
  ...
  if (mongoose.Types.ObjectId.isValid(req.params.restID)) {
    console.log('Shopper id is valid. params = ', req.params)
    return true
...
//   // do logging
//   console.log('Something is happening shoppers router')
...
router.route('/users/shoppers')
  .post(guestUp, function (req, res) { // anyone can create a new shopper account
     ...
     * phone
     * -THIS user-
     * account_type (defaults to Shopper)
     * address.street
     * address.apt
     * address.city
     * address.state
     ...
     * orders.current (array)
     * orders.previous (array)
  ...
  .get(dropperUp, function (req, res) { // vulcan, admin and dropper can get ALL shoppers
    console.log('GET on /users/shoppers')
    // Shopper.find({ 'account_type': 'shopper' },
    //   'name.first name.last email phone address.street address.apt address.city address.state address.zip address.instructions',
    //   function (err, users) {
    Shopper.find({ 'account_type': 'shopper' }, function (err, users) {
      if (err) {
        res.send(err)
      } else {
        res.json(users)
...
/**
 * Single Shopper (GET, PUT and DELETE)
 */
router.route('/users/shoppers/:_id')
// get the user with that id (accessed at GET http://localhost:8080/api/users/:_id)
  .get(idOrVulcan, function (req, res) { // CHANGE THIS TO idOrDropperUP
    console.log('GET on /users/shoppers/', req.params._id)
    if (validResourceID(req, res)) {
      Shopper.findById(req.params._id, function (err, user) {
  ...
  .put(idOrVulcan, function (req, res) { // CHANGE THIS TO idOrAdminUP
    console.log('PUT on /users/shoppers/', req.params._id)
    ...
    if (validResourceID(req, res)) {
      Shopper.findById(req.params._id, function (err, user) {
        if (err) {
          ...
          if (req.body.address.instructions) {
            // update zipcode
  ...
  .delete(adminUp, function (req, res) { // only vulcan or admin can remove shopper from database
    console.log('DELETE on /users/shoppers/', req.params._id)
    if (validResourceID(req, res)) {
      Shopper.remove({
        _id: req.params._id
      }, function (err, user) {
